 	...
 	class BaseEditorController extends ActionController {
		...
		public function PrintSummary($pa_options=null) {
			...
			try {
				...
				// libis_start
				// Mag verwijderd worden als het probleem met lege velden die getoond worden opgelost is met multiple occurences
				...
				if(preg_match('/Afmeting opmerking:[\s]+<br\/>/', $vs_content)) {
					$temp_replace = preg_replace('/Afmeting opmerking:[\s]+<br\/>/','<br/>',$vs_content); 
					$vs_content = $temp_replace;
				}
				if(preg_match('/\/[\s]+<br\/>/', $vs_content)) {
					$temp_replace = preg_replace('/\/[\s]+<br\/>/','<br/>',$vs_content);
					$vs_content = $temp_replace;
				}
				// in commentaar gezet om te vervagnen door de DOMPDF implementatie
				...
				$o_pdf->set_paper("A4", "portrait");
                                ini_set('memory_limit','2048M');
                                set_time_limit(600000);
                                //$fp = fopen('/www/libis/web/collectiveaccess/crkc_media/htmlPrint.txt', 'w');
