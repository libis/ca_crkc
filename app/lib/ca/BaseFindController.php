  ...
  */
 		...
 		public function Index($pa_options=null) {
			...
			if (!sizeof($va_display_list)) {
				if ($vs_idno_fld = $t_model->getProperty('ID_NUMBERING_ID_FIELD')) {
                                        $va_multipar_id = new MultipartIDNumber($this->ops_tablename, '__default__', null, $t_model->getDb()); //// libis
					$va_display_list[$this->ops_tablename.'.'.$vs_idno_fld] = array(
						...
						'settings' => array(),
						'allowInlineEditing' => $va_multipar_id->isFormatEditable($this->ops_tablename), //true,  //// libis,
	...
	// 						}
			...
			else
			{
                ...
                $va_pdf_options['orientation'] = isset($va_settings['orientation']) ? $va_settings['orientation'] : 'portrait' ;
                $va_pdf_options['page_format'] = isset($va_settings['page_format']) ? $va_settings['page_format'] : 'A4' ;
                ...
                case '_pdf':
                    ...
                    $vs_header = $this->render('Results/'.$this->ops_tablename.'_pdf_results_long_html_header.php');
                    ...
                    if($this->isQueuingServer($va_settings) === true){
                        ...
                        $o_pdf = new DOMPDF();
                        $o_pdf->set_paper($vs_page_format, $vs_orientation);
                        $o_pdf->load_html($vs_content, $vs_enocoding);
                        $o_pdf->render();
            ...
            case '_pdfall':
                header("Content-Disposition: attachment; filename=export_results.pdf");
                header("Content-type: application/pdf");
                $vs_content = $this->render('Results/'.$this->ops_tablename.'_pdf_results_long_all_html.php');
                $vs_header = $this->render('Results/'.$this->ops_tablename.'_pdf_results_long_all_html_header.php');
                //Sam: Mag verwijderd worden als het probleem met lege velden die getoond worden opgelost is met multiple occurences
                // Kijken of afmeting bestaat op het einde zonder waarden, want dan moet het straks afgekapt worden
                if(preg_match('/hoogte:[\s]+\//', $vs_content)) {
                    $temp_replace = preg_replace('/hoogte:[\s]+\//', '', $vs_content);
                    $vs_content = $temp_replace;
                }
                if(preg_match('/breedte:[\s]+\//', $vs_content)) {
                    $temp_replace = preg_replace('/breedte:[\s]+\//', '', $vs_content);
                    $vs_content = $temp_replace;
                }
                if(preg_match('/diepte:[\s]+\//', $vs_content)) {
                    $temp_replace = preg_replace('/diepte:[\s]+\//', '', $vs_content);
                    $vs_content = $temp_replace;
                }
                if(preg_match('/diameter:[\s]+\//', $vs_content)) {
                    $temp_replace = preg_replace('/diameter:[\s]+\//', '', $vs_content);
                    $vs_content = $temp_replace;
                }
                if(preg_match('/lengte:[\s]+\//', $vs_content)) {
                    $temp_replace = preg_replace('/lengte:[\s]+\//', '', $vs_content);
                    $vs_content = $temp_replace;
                }
                if(preg_match('/gewicht:[\s]+Afmeting/', $vs_content)) {
                    $temp_replace = preg_replace('/gewicht:[\s]+Afmeting/','Afmeting',$vs_content);
                    $vs_content = $temp_replace;
                }
                if(preg_match('/Afmeting opmerking:[\s]+<\/span>/', $vs_content)) {
                    $temp_replace = preg_replace('/Afmeting opmerking:[\s]+<\/span>/','</span>',$vs_content);
                    $vs_content = $temp_replace;
                }
                if(preg_match('/\/[\s]+<\/span>/', $vs_content)) {
                    $temp_replace = preg_replace('/\/[\s]+<\/span>/','</span>',$vs_content);
                    $vs_content = $temp_replace;
                }
