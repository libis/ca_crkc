...
//														$o_lucene_query_element->getTerm()->field = $vs_table.'.'.$vs_fld_num;
							                                                        ...
							                                                        //libis_start
                                                        							// A pull requst has been accepted for this fix.
		...
		$vo_http_client->setConfig(array(
				'timeout'      => 30));
		...
		//file_put_contents(__CA_BASE_DIR__.'/tmp/solrsearch.txt', print_r("query ===> ".utf8_decode($ps_search_expression), true)."\n", FILE_APPEND);
	...
	public function flushContentBuffer() {
		...
		foreach($va_post_xml as $vs_core => $vs_post_xml) {
			...
			//file_put_contents(__CA_BASE_DIR__.'/tmp/solrsearchdata.txt', print_r($vs_post_xml, true)."\n", FILE_APPEND);
