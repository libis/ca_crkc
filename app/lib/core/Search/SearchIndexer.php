							...
							switch($vs_rel_field){
								...
								default:
									if ($vb_is_attr) {
										...
										// This exclusion list was prepared in commitRowIndexing function as a part of solution to remove indexing of removed items
										// from related tables( mainly for removing items from a set).
					                                        if(array_key_exists($vn_related_tablenum, $pa_exclusion_list)){
                                        					    $temp_fields = $pa_exclusion_list[$vn_related_tablenum];
					                                            $temp_field_code = 'I'.$this->opo_datamodel->getFieldNum($vs_related_table, $vs_rel_field);
                                        					    if(array_key_exists($temp_field_code,$temp_fields) && $temp_fields[$temp_field_code] === $vs_fld_data )
					                                                continue;
                                        					}
					                                        //libis_end
			...
			if (!$vb_can_do_incremental_indexing) {
				...
				foreach($this->opa_dependencies_to_update as $va_item) {
					...
					$o_indexer->indexRow($va_item['field_table_num'], $va_item['field_row_id'], $va_item['field_values'], true);
			                ...
			                //libis_start
					// Above indexing(indexRow) is not indexing all related rows. For example in case of removing an item from a set needs to update
					// index of related rows in tables ca_sets and ca_objects. Above indexing only updates index of ca_sets. Following code will update index
					// of the remaining related row.
