...
//libis_start
 				# --- headings for user_groups
 				for ($i = 1; $i <= 20; $i++) {
 					$va_row[] = "user_group_".$i;	
 				}
//libis_start
...
// libis_start
					# --- user_groups
					$user_id = $qr_users->get("ca_users.user_id");
					$qr_user_groups = $o_db->query("SELECT link.group_id, name FROM ca_users_x_groups link  
									INNER JOIN ca_user_groups AS groups
									WHERE link.group_id = groups.group_id
									AND link.user_id = $user_id
									ORDER BY name DESC");
					if($qr_user_groups->numRows()){
						$aantal = $qr_user_groups->numRows();
						while($qr_user_groups->nextRow()){
							$va_row[] = $qr_user_groups->get("name");
						}
						$rest = 20 - $aantal;
					}else{
						$rest = 20;
