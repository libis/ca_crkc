			...
			for(i=minImageImage; i <= maxImageIndex; i++) {
				if (jQuery("#" + that.scrollingImageIDPrefix + i).length == 0) {
					...
					if (!that.noImageLink) { linkOpenTag = '<a href="' + that.scrollingImageList[i].link + '" '+(that.scrollingImageList[i].onclick ? 'onclick="' + that.scrollingImageList[i].onclick + '"' : '') + ' '+(that.scrollingImageList[i].rel ? 'rel="' + that.scrollingImageList[i].rel + '"' : '') + '>'; linkCloseTag = '</a>'} else { linkOpenTag = linkCloseTag = ""; }
                                        // libis width/height: 100%; (was ' + that.scrollingImageList[i].height + ') width= 170px (was ' + that.scrollingImageList[i].width + ')
					jQuery('#' + that.container).append('<div class="' + that.scrollingImageClass + '" id="' + that.scrollingImageIDPrefix + i + '" style="'+horizCentering + ' ' + vertCentering +'; height: 100%;">'+ linkOpenTag +'<img src="' + that.scrollingImageList[i].url+ '" style="max-height:250px; max-width:170px;" border=\'0\'>'+ linkCloseTag +'</div>');
