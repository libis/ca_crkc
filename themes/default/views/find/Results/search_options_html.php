...
<?php
        ...
        foreach($va_sortable_elements as $vn_element_id => $va_sortable_element) {
            ...
            // if search is based on type id filter sortable elements
                ...
                foreach($element_type_restrictions as $value){
                    if(is_array($value)){
                        foreach($value as $key => $temp_value){
                            if($key == $vs_type_id || empty($key) === true )
                                $sortable_items_1[$vs_table_name.'.'.$va_sortable_element['element_code']] = $va_sortable_element['display_label'];
                        }
                    }
                }
            }
        ...
        if(is_array($this->getVar("sorts")) && (sizeof($this->getVar("sorts")) > 0)){
            ...
            // libis_start_nm
            $sortable_items_2 = $this->getVar('sorts');
            $sortable_items = array_merge($sortable_items_2, $sortable_items_1);
            // libis_end_nm

//			foreach($this->getVar("sorts") as $vs_sort => $vs_option){
			foreach($sortable_items as $vs_sort => $vs_option){         // libis
